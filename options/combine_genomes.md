...
## combine genomes options

|Tool Suite|Pipeline|Action|
|---|---|---|
|HiFiRe3|combine|genomes|
...
```
combine: Concatenate two reference genomes for use with mixed-species samples

genomes: concatenate two reference genomes for use with mixed-species samples

combine-genomes:
  -1,--genome1         <string> the first/primary reference genome to concatenate [hs1]
  -2,--genome2         <string> the second/secondary reference genome to concatenate [dm6]
  -G,--genomes-dir     <string> directory to be filled with sub-directories, one per reference genome [${MDI_DIR}/resources/genomes]

digestion:
  -Y,--min-priority-level   <integer> exclude blunt REs with usability/priority level < -Y [4]
