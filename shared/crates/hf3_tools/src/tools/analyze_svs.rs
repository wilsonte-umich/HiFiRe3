...
pub fn main() -> Result<(), Box<dyn Error>> {
    ...
    cfg.set_string_env(&[FINAL_JXNS_FILE, SV_READ_PATHS_FILE, DISTAL_ALNS_FILE]);
    ...
    let mut read_paths: Vec<SvReadPath> = Vec::with_capacity(100_000);
    let mut distal_alns: Vec<AlignmentSegment> = Vec::with_capacity(100_000);
    ...
    if jxns.len() > 1 {
        let jxns = fuzzy_match_junctions(
            jxns,
            &chroms,
            *w.cfg.get_usize(GROUP_BREAKPOINT_DISTANCE),
                  *w.cfg.get_usize(GROUP_STEM_DISTANCE) as u32,
        )?;
    }
    // write_sorted_with_count(
    //     jxns, 
    //     w.cfg.get_string(FINAL_JXNS_FILE)
    // )?;
