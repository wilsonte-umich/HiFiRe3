...
use genomex::sam::{SamRecord, junction::JunctionType};
...
pub fn process_chrom <'a>(
    chroms:        &'a Chroms,
    rx_chrom:      Receiver<(String, usize)>,
    ...
    for (_chrom_name, chrom_index) in rx_chrom.iter() {
        ...
        // all reads are on-target and have first alignment on chrom_name
        ...
        let mut tool = JunctionTool {
            ...
            chroms,
            header,
            first_alns:    Vec::with_capacity(100_000), // TODO: predict this capacity more accurately
            tx_jxn:        &tx_jxn,
...
fn process_alns(
    ...
    // get outer nodes from first alignment
    let ordered_outer_nodes = SamRecord::sam_tag_to_paired_nodes(
        &tags::get_tag_str(&alns[0], OUTER_NODES), 
        true
    );

    // not the same as READ_HAS_PASSED_JXN or n_alns > 1
    // traversal failures, and only traversal failures, are skipped here
    ...
    for aln_i in 0..=max_aln_i {
        ...
        if aln_i < max_aln_i {
            ...
            if jxn_failure_flag & JxnFailureFlag::TraversalDelta as u8 == 0 { // only record juctions that passed traversal delta
        ...
        let aln_segment = AlignmentSegment::new(
            alns,
            aln_i,
            prev_jxn_type | this_jxn_type, // jxn_types flanking this aln
            tool,
        );
        if aln_i == 0 {
    ...
    if has_committed_jxn {
        let read_path = SvReadPath::new(alns, &tool.header, tool.chroms);
