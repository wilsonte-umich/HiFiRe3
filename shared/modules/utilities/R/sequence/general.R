...
getMaxInsertSize <- function(){
    pmis <- as.integer(env$PLATFORM_MAX_INSERT_SIZE)
     mis <- as.integer(env$MAX_INSERT_SIZE)
    if(is.na(pmis)) stop("missing or zero or non-integer value for PLATFORM_MAX_INSERT_SIZE")
    if(is.na( mis)) stop("missing or zero or non-integer value for MAX_INSERT_SIZE")
    min(pmis, mis);
}
