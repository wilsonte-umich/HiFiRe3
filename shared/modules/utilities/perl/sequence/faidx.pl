...
#----------------------------------------------------------
# sequence retrieval from indexed reference genome loaded into memory
#----------------------------------------------------------
...
sub getRefSeq { 
    my ($chrom, $nodePos, $len, $nodeN, $rc) = @_;
    my ($pos1, $pos2);
    if($nodeN == 1){
        ($pos1, $pos2) = $rc ? ($nodePos, $nodePos + $len) : ($nodePos - $len, $nodePos);
    } else {
        ($pos1, $pos2) = $rc ? ($nodePos - $len, $nodePos) : ($nodePos, $nodePos + $len);
    }
    $pos1 < 1 and $pos1 = 1;
    $pos2 > $faidx{$chrom}[FAIDX_LENGTH] and $pos2 = $faidx{$chrom}[FAIDX_LENGTH];
    $pos1--;
