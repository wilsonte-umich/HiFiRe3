#----------------------------------------------------------------------
# sv_arcs is a track type for plotting SV node pairs as semicircle "jumps" between them
# expects:
...
# each source may have one or more samples, as a result of multi-sample find actions in the relevant pipelines
#----------------------------------------------------------------------

# constructor
new_sv_arcsTrack <- function(trackId, expandReactive) {
    list(
        click  = FALSE, # TRUE,
        hover  = FALSE,
        brush  = FALSE,
        items  = TRUE,
        expand = FALSE, # expandReactive,
        expand2 = FALSE, # TRUE,
...
build.sv_arcs_track <- function(track, reference, coord, layout, trackBuffer, loadFn, idCol, 
    ...
    mai <- NULL
    image <- mdiTrackImage(layout, height, message = "sv_arcs", function(...){
