...
# input:
#   final junctions from many samples on STDIN, presorted by chromIndex1_1,strandIndex0_1,refPos1_1
# output:
#   final junctions now further aggregated across samples on STDOUT

# initialize reporting
our $script = "compare_SVs";
...
printCount(commify($nOutputJxns), 'nOutputJxns', 'output junctions after fuzzy node matching across samples');
...
sub processJunctions_3 {
    ...
    # UJXN_IS_EXPECTED..UJXN_IS_EXCLUDED_2 also persist as is for this best junction (genome-level values should be the same)
    @jxns_3 = sort { $$b[UJXN_N_OBSERVED] <=> $$a[UJXN_N_OBSERVED] } @jxns_3; 

    # collect the samples that called this junction
    my @samples = map { $$_[CMP_SAMPLES] =~ m/^,(.+),$/; $1 } @jxns_3;
    $jxns_3[0][CMP_N_SAMPLES] = scalar @samples;
    $jxns_3[0][CMP_SAMPLES] = "," . join(",", @samples) . ","; # in descending order of N_OBSERVED

    # UJXN_N_OBSERVED, UJXN_BKPT_COVERAGE_1, and UJXN_BKPT_COVERAGE_2 summed over all aggregating junctions
    foreach my $discardedJxn(@jxns_3[1..$#jxns_3]){
        $jxns_3[0][UJXN_N_OBSERVED] += $$discardedJxn[UJXN_N_OBSERVED];
        $jxns_3[0][UJXN_BKPT_COVERAGE_1] += $$discardedJxn[UJXN_BKPT_COVERAGE_1];
        $jxns_3[0][UJXN_BKPT_COVERAGE_2] += $$discardedJxn[UJXN_BKPT_COVERAGE_2];
