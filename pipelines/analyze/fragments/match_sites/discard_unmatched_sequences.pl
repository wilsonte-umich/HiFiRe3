...
sub processQName {
    ...
    my $dist3 = abs( ($$aln3[S_FLAG] & _REVERSE) ? $$aln3[SITE_DIST_2] - $clip3 : $$aln3[SITE_DIST_2] + $clip3 );
    if($isEndToEnd){
    ...
    # do not use EVENT_HAS_SV here, need to establish connection between ends according to reference
    my $hasSV = @alns > 1;
    foreach my $aln(@alns){
        $isEndToEnd and !$hasSV and @$aln[SEQ_SITE_INDEX1_2..SEQ_SITE_POS1_2] = @$aln3[SITE_INDEX1_2..SITE_POS1_2];
