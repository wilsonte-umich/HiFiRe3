...
# scope:
#   this script applies to non-RE-cleaved fragment libraries such as tagFree
# action:
...
##############
# foreach my $chrom("chr18_mm10", "chr19_mm10"){
# foreach my $chrom("chr21_hs1", "chr22_hs1"){
    ...
    while(my $line = <$readH>){
        ...
        while(my $aln = <$chromH>){
            ...
            if($prevRead and $prevRead ne $aln[S_QNAME]){
                $refPos1_5 = getRefPos1_5($alns[0]);
                my $binI = int($refPos1_5 / INDEX_BIN_SIZE);
                my $key = indexRead();
                $key and $index[ $binI ]{ $key }++;
        ...
        foreach my $binI(0..$#index){
            ...
            foreach $varKey(sort { 
                substr($a, 0, N_CHAR_ALN_SORT) cmp substr($b, 0, N_CHAR_ALN_SORT) or  # sort of zeroPaddedRefPos1_5::strandIndex0
                $$vars{$b} <=> $$vars{$a} # most frequent variant pattern sorts first for a given refPos1_5+strandIndex0
            } keys %$vars){
